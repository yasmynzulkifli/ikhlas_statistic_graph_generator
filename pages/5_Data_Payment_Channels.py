...
st.set_page_config(
    page_title="Data Payment Channels",
    page_icon="💳",
    layout="wide",
...
def reset():
    st.session_state.pc_stage        = "upload"
    st.session_state.pc_df           = None
    st.session_state.pc_file         = ""
    st.session_state.pc_uploader_key += 1
...
def load_df(file_bytes, fname):
    ...
    required = ["HOTEL","SALES","CASH","OTA","CC","QR",
                "S/D % Sales","S/D % Cash","S/D % OTA","S/D % CC","S/D % QR"]
...
def draw_chart(df: pd.DataFrame):
    ...
    for col, label, ck in LINE_COLS:
        vals  = df[col].values.astype(float)
        color = COLORS[ck]
        ax2.plot(x, vals, color=color, linewidth=1.0,
                 marker="o", markersize=2, zorder=5, linestyle="--")
        line_handles.append(
            mlines.Line2D([], [], color=color, linewidth=1.0,
    ...
    ax1.set_title("Payment Channels & S/D % — All Hotels",
...
# PAGE
# ═══════════════════════════════════════════════════════════════════════════════
st.title("💳 Data Payment Channels")
st.caption("Upload the payment channel report to view grouped bar chart by hotel with S/D % lines.")
st.markdown("---")
...
if st.session_state.pc_stage == "upload":
    ...
    if proceed:
        ...
        st.session_state.pc_file  = up.name
        st.session_state.pc_stage = "graph"
        st.rerun()

# ── STAGE: GRAPH ──────────────────────────────────────────────────────────────
elif st.session_state.pc_stage == "graph":
    df = st.session_state.pc_df
    st.caption(f"File: **{st.session_state.pc_file}** — {len(df)} hotels")
    ...
    with st.container(border=True):
        ...
        cols[4].metric("Total QR",    f"RM {df['QR'].sum():,.2f}")
    ...
    dl1.download_button("⬇️ Download Chart", buf,
                        file_name="payment_channels_report.png", mime="image/png",
